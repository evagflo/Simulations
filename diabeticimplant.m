function [g] = diabeticimplant (N)
    ...
    %variables
        ...
        g_ss= g_s(n-1)/1000;
    %     Ka(n) =(900 + (7000 / (1 + 200 * (g_s(n-1))^ 2)))*0.005 ;%* 0.001;
        Ka(n) =(900 + (7000 / (1 + 200 * (g_ss)^ 2)))* 0.001;
        ma(n) = ma(n-1) +(Ka(n) -delta_a * ma(n-1) - K1 * ma(n-1) * mb(n-1) + K_1 * mab(n-1) - K2 * mc(n-1) * ma(n-1) + K_2 * mca(n-1)) * dt;
        mb(n)= mb(n-1) +(Kb - delta_b * mb(n-1) - K1 * ma(n-1) * mb(n-1) + K_1 * mab(n-1))*dt;
        mab(n)= mab(n-1) + (K1 * ma(n-1) * mb(n-1) - K_1 * mab(n-1) - delta_ab * mab(n-1))*dt;
        mc(n)= mc(n-1) +(Kc - delta_c * mc(n-1) - K2 * ma(n-1) * mc(n-1) + K_2 * mca(n-1)  )*dt;
        mca(n) = mca(n-1) + (K2 * ma(n-1) * mc(n-1) - K_2 * mca(n-1) - delta_ca * mca(n-1))*dt;
    ...
    % % % plot(time, g)
    % % % hold on
    % % % plot( [time(1) time(end)],[180 180], 'r')
    % % % hold on
    % % % plot( [time(1) time(end)],[70   70], 'g')
