function [g,time] = diabeticsubjectsensor(gb,thr)% GIM Model Eva
% bona 11/3/16
%forward-euler
%%close all
...
output_cell=zeros(1,total_time);
...
%variables
    ...
    %rate of appearence of insulin
    ri(n)= (ka1 * s1(n-1) + ka2 *s2(n-1));
    %% cellular glucose sensor
    %input= g_s= intesrtitial glucose
    %output=nonmonomeric insulin
    % hill function fitting glucose data from sensor_arq1.m -> 19.36*(glucose_levels(m))^ 2 / (0.0100+ (glucose_levels(m))^ 2) ;
    output_cell(n)= output_cell(n-1) + (N*((g_s(n-1))^ 2 / (thr+ (g_s(n-1))^ 2))) *dt;
    %insulin kinetics
    he(n)= -m5 * s(n) + m6;
    m3(n)=(he(n)* m1)/(1- he(n));
...
%     % glucose utilization
    ...
    %nonmonomeric insulin in subcutaneous space
    s1(n)= s1(n-1) + dt* (output_cell(n) -(ka1+kd)*s1(n-1));
    %monomeric insulin in subcutaneous space
    s2(n)= s2(n-1) + dt*( kd*s1(n-1) - ka2*s2(n-1));
    
    %%glucose subsytem
    %glucose plasma
    p(n)= gp(n-1) + dt* (egp(n) + ra(n) - uii(n) - e(n) - k1*gp(n-1) + k2*gt(n-1));
    %glucose tissue
    gt(n)= gt(n-1) + dt*( k1*gp(n-1) - k2*gt(n-1) - uid(n));
    % glucose sensor sigbl-> subcutaneous glucose 8intesrtitial)
...
% 
% figure(2)
% plot(time, output_cell)
% hold on
% plot(time, g_s)
% legend('outputcell','g_s')
% xlabel('time (h)')
