...
%     Ka(n) =(900 + (7000 / (1 + 200 * (g_s(n-1))^ 2))) * 0.001;
%     ma(n) = ma(n-1) +(Ka(n)* Ga(n-1) -delta_a * ma(n-1) - K1 * ma(n-1) * mb(n-1) + K_1 * mab(n-1) - K2 * mc(n-1) * ma(n-1) + K_2 * mca(n-1)) * dt;
%     mb(n)= mb(n-1) +(Kb * Gb(n-1) - delta_b * mb(n-1) - K1 * ma(n-1) * mb(n-1) + K_1 * mab(n-1))*dt;
%     mab(n)= mab(n-1) + (K1 * ma(n-1) * mb(n-1) - K_1 * mab(n-1) - delta_ab * mab(n-1))*dt;
%     mc(n)= mc(n-1) +(Kc * Gc(n-1) - delta_c * mc(n-1) - K2 * ma(n-1) * mc(n-1) + K_2 * mca(n-1)  )*dt;
%     mca(n) = mca(n-1) + (K2 * ma(n-1) * mc(n-1) - K_2 * mca(n-1) - delta_ca * mca(n-1))*dt;
    
    outputcell(n)= equacionssensor(g_s(n-1),n,total_time,dt);
